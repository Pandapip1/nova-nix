/* GNU diffutils 3.8 on ntlibc: what ./configure would have written.
   Generated from lib/config.hin by answering every #undef by hand; see
   the head of build.kaem and default.nix for the ones that are
   load-bearing.  Everything not answered below is answered `no'.  */
...
#define PACKAGE_STRING "GNU diffutils 3.8"
...
/* Two macros gnulib injects into the headers it generates rather than into
   config.h, and the only two this build turns out to need.  gnulib keeps
   them in lib/arg-nonnull.h and lib/stdio.in.h and splices them into every
   generated .h; with no generated headers here they have to be stated once,
   and config.h -- included first by every file -- is where.

   Both are diagnostic-only.  _GL_ARG_NONNULL tells GCC which pointer
   arguments may not be null and _GL_ATTRIBUTE_FORMAT_PRINTF_STANDARD tells
   it to typecheck a printf format; tcc does neither kind of checking, so
   these are the expansions gnulib itself produces for a compiler without
   __attribute__.  lib/error.c is the only user of either -- grep for them
   over the whole tree finds error.c, arg-nonnull.h and stdio.in.h and
   nothing else.

   The interfaces this port has to ADD to ntlibc rather than merely name are
   in fnmatch.h, langinfo.h, locale.h, string.h, wchar.h, time.h and
   sys/stat.h beside this file, where gnulib would have generated them from
   the matching .in.h.  */
#ifndef _GL_ARG_NONNULL
# define _GL_ARG_NONNULL(params) /* empty */
#endif
#ifndef _GL_ATTRIBUTE_FORMAT_PRINTF_STANDARD
# define _GL_ATTRIBUTE_FORMAT_PRINTF_STANDARD(f, a) /* empty */
#endif

/* ---------------------------------------------------------------------
   And two of the macros src/system.h says are "POSIX-compliant defaults ...
   meant to be overridden by hand in config.h as needed".  This is the one
   part of this port that is about NT's filesystem rather than about its C
   library, and it is the only wrong ANSWER this build produced before it was
   found -- everything else was a compile or a link error.

   same_file
       src/system.h's default is `st_ino equal and st_dev equal', with the
       contract "Answer -1 if unknown" written above it and every one of the
       three callers spelled `0 < same_file (...)' to honour that.  ntlibc
       fills st_ino from NTFS's FileInternalInformation, which is a real and
       distinct number for every regular file and directory -- but it returns
       a memset-to-zero struct stat for a pipe, a console and a character
       device, and it falls back to zero when the query fails.  Two such
       handles therefore compare EQUAL under the default, at st_ino 0 and
       st_dev 0, and so do their st_rdev's under same_special_file.

       That is not theoretical and it is not a small bug.  It made `cmp a b'
       print NOTHING whenever its standard output was a pipe: cmp stats
       stdout, stats NULL_DEVICE, and if they are the same file it switches
       to type_no_stdout and reports the difference through the exit status
       alone (src/cmp.c, "If output is redirected to the null device, we can
       avoid some of the work").  Both stats came back all-zero, so cmp
       decided every pipe was /dev/null.  The exit status stayed right, which
       is exactly how a bug like this survives a smoke test.  The same
       comparison decides whether `diff' reads its two arguments at all
       (src/diff.c: "The two named files are actually the same physical
       file.  We know they are identical without actually reading them") and
       whether `diff -r' recurses into a directory entry twice (src/dir.c).

       So a zero inode is treated as what it is -- unknown -- and answered
       -1, which is the escape hatch the interface already documents.  A file
       ntlibc can identify is still compared the same way it was.

   NULL_DEVICE
       is "/dev/null" by default, and NT's null device is NUL.  Under wine
       the POSIX spelling is worse than useless: Z: maps the host's root, so
       stat("/dev/null") succeeds and hands cmp the HOST's null device to
       compare its output against.  With same_file fixed the answer is right
       either way -- both are character devices, so both are unknown -- but
       naming the device this platform actually has is the honest half of
       the same finding.  */
