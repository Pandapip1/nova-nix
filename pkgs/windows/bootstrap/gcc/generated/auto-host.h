/* auto-host.h for this chain's gcc-4.6.4 (host = target = i686-pc-mingw32,
 * ntlibc): audited, not regenerated for this host.
 *
 * Provenance: gcc-4.6.4's real ./configure was run on an ordinary Linux
 * host (host=build=x86_64-linux, see ../sources.nix's sibling commit and
 * ../../../binutils/default.nix's own "What generated/ actually is" for
 * why a real configure run, off-chain, is how this project gets accurate
 * generated output without paying the exec-storm cost on ntlibc itself).
 * That run's own build/gcc/auto-host.h is a Linux/glibc/LP64 answer set,
 * not this chain's actual host -- every line below has been checked
 * against that fact and, where it mattered, corrected against ntlibc's
 * real headers at /home/gavin-john/Projects/ntlibc/include (a read-only
 * reference clone owned by a peer session; corrections here are audit
 * findings, not edits to that repo).
 *
 * What was actually corrected (12 lines, each tagged "ntlibc audit:"
 * in place below) -- the governing rule from every earlier package in
 * this chain is that a wrong HAVE_ answer is not a style nit, it turns a
 * real declaration into syntax garbage or vice versa, so each of these
 * is a concrete, checked finding, not a guess:
 *
 *   SIZEOF_LONG, SIZEOF_VOID_P: the reference host is x86_64-linux (LP64:
 *     long and void* are 8 bytes); this chain's actual host is
 *     i686-pc-mingw32 (ILP32: both are 4 bytes). This is an architecture
 *     fact, not a libc fact, but it is exactly the kind of silently
 *     wrong answer this audit exists to catch -- gcc's own hwint.h and
 *     host-hooks code size internal integers off these.
 *
 *   HAVE_MALLOC_H, HAVE_SYS_MMAN_H, HAVE_LANGINFO_H, HAVE_ICONV_H:
 *     none of these four headers exist under ntlibc/include (checked
 *     directly: `ls ntlibc/include`, `ls ntlibc/include/sys`) -- flipped
 *     to undef.
 *
 *   HAVE_MMAP, HAVE_MMAP_ANON, HAVE_MMAP_DEV_ZERO, HAVE_MMAP_FILE:
 *     downstream of HAVE_SYS_MMAN_H -- ntlibc has no mmap() at all (no
 *     <sys/mman.h> to declare it in, and nothing in libc.a implements
 *     it). Left defined, ggc-page.c and libiberty's xmmap-based paths
 *     would compile against a declaration that does not exist and fail
 *     to link, or worse, implicitly-declare and miscompile under a
 *     compiler this fussy about it -- undef'd.
 *
 *   HAVE_LANGINFO_CODESET: downstream of HAVE_LANGINFO_H -- undef'd.
 *
 *   HAVE_ICONV: downstream of HAVE_ICONV_H -- ntlibc has no iconv.h and
 *     no iconv() -- undef'd.
 *
 *   HAVE_WCSWIDTH: checked ntlibc/include/wchar.h directly -- no
 *     wcswidth() (nor wcwidth(), which it would be built from) is
 *     declared there. Left defined, this would have been exactly the
 *     "declared 1, ntlibc doesn't have it" trap the governing rule
 *     warns about -- undef'd.
 *
 * Explicitly verified present and left AS the reference run already had
 * them (checked directly against ntlibc/include, not assumed): fork(),
 * vfork(), sysconf(), getrlimit()/setrlimit()/getrusage() (all three
 * real, in sys/resource.h, with setrlimit() genuinely enforcing what NT
 * can enforce -- see that header's own long comment), times() (struct
 * tms + times(), sys/times.h), gettimeofday() (sys/time.h), setlocale()
 * and LC_ALL/LC_MESSAGES (locale.h -- this is the same LC_ALL the
 * "-DLC_ALL=" bad pattern from an earlier package would have destroyed;
 * here it is genuinely defined and must stay that way), mbstowcs()
 * (stdlib.h), dlopen/dlsym/dlclose/dlerror (dlfcn.h -- HAVE_DLFCN_H=1 is
 * correct), kill() (signal.h), strsignal() (string.h -- also the same
 * "-Dstrsignal" bad-pattern name from an earlier package; here it is
 * genuinely declared and HAVE_STRSIGNAL/HAVE_DECL_STRSIGNAL=1 is
 * correct as-is), atoll() and clock() (stdlib.h/time.h), the entire
 * *_unlocked stdio family except fprintf_unlocked (stdio.h -- matches
 * the reference's own HAVE_FPRINTF_UNLOCKED undef, which is also
 * correct: glibc doesn't have it either, and this is not an ntlibc gap),
 ...
 *     measurement (see ../../gcc's own scoping commit) rather than the
 *     minimal --disable-shared --disable-nls --disable-libssp
 *     --disable-libquadmath ... set build.kaem will actually pass (this
 *     package only vendors gcc-core, so libquadmath's source isn't even
 *     present -- ENABLE_LIBQUADMATH_SUPPORT=1 here is already stale on
 *     that basis alone). These need to be re-derived once build.kaem's
 *     exact configure-flag set is chosen, not audited against ntlibc.
 *
 *   - HAVE_EXT_HASH_MAP / HAVE_TR1_UNORDERED_MAP / HAVE_UNORDERED_MAP:
 *     C++ standard-library header checks. Dead weight for this package
 *     regardless of their answer -- this build compiles gcc with tcc (a
 *     C compiler) and vendors gcc-core only, so no C++ source is ever
 *     compiled here. Left as the reference produced them.
 *
 * See ../default.nix (not yet written) for how AS_FOR_TARGET/LD_FOR_TARGET
 * and the exact configure-flag set get settled, which is what the two
 * deferred categories above are waiting on.
 */
...
#ifndef USED_FOR_TARGET
/* #undef HAVE_ICONV */  /* ntlibc audit: no iconv.h/iconv() in ntlibc */
#endif


/* Define to 1 if you have the <iconv.h> header file. */
#ifndef USED_FOR_TARGET
/* #undef HAVE_ICONV_H */  /* ntlibc audit: no iconv.h in ntlibc/include */
#endif
...
#ifndef USED_FOR_TARGET
/* #undef HAVE_LANGINFO_CODESET */  /* ntlibc audit: no langinfo.h, so no nl_langinfo(CODESET) */
#endif


/* Define to 1 if you have the <langinfo.h> header file. */
#ifndef USED_FOR_TARGET
/* #undef HAVE_LANGINFO_H */  /* ntlibc audit: no langinfo.h in ntlibc/include */
#endif
...
#ifndef USED_FOR_TARGET
/* #undef HAVE_MALLOC_H */  /* ntlibc audit: no malloc.h in ntlibc/include */
#endif
...
#ifndef USED_FOR_TARGET
/* #undef HAVE_MMAP */  /* ntlibc audit: no mmap()/sys/mman.h at all in ntlibc */
#endif


/* Define if mmap with MAP_ANON(YMOUS) works. */
#ifndef USED_FOR_TARGET
/* #undef HAVE_MMAP_ANON */  /* ntlibc audit: depends on HAVE_MMAP, which is undef here */
#endif


/* Define if mmap of /dev/zero works. */
#ifndef USED_FOR_TARGET
/* #undef HAVE_MMAP_DEV_ZERO */  /* ntlibc audit: depends on HAVE_MMAP, which is undef here */
#endif


/* Define if read-only mmap of a plain file works. */
#ifndef USED_FOR_TARGET
/* #undef HAVE_MMAP_FILE */  /* ntlibc audit: depends on HAVE_MMAP, which is undef here */
#endif
...
#ifndef USED_FOR_TARGET
/* #undef HAVE_SYS_MMAN_H */  /* ntlibc audit: no sys/mman.h in ntlibc/include */
#endif
...
#ifndef USED_FOR_TARGET
/* #undef HAVE_WCSWIDTH */  /* ntlibc audit: no wcswidth() declared in ntlibc/include/wchar.h (wcwidth() is also absent) */
#endif
...
#ifndef USED_FOR_TARGET
#define SIZEOF_LONG 4  /* ntlibc audit: ILP32 target host (i686-pc-mingw32), not the LP64 x86_64-linux reference host */
#endif
...
#ifndef USED_FOR_TARGET
#define SIZEOF_VOID_P 4  /* ntlibc audit: ILP32 target host (i686-pc-mingw32), not the LP64 x86_64-linux reference host */
#endif
