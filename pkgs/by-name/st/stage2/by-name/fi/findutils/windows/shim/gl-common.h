/* gnulib's own boilerplate, exactly as its m4 would have written it into
 * config.h.
 *
 * This is NOT a set of answers about the target.  gnulib's gl_COMMON_BODY
 * (gl/m4/gnulib-common.m4) emits eight hundred lines of AH_VERBATIM into
 * every config.h it generates: _GL_GNUC_PREREQ, _Noreturn, _GL_CMP,
 * gl_va_copy and the whole _GL_ATTRIBUTE_* vocabulary that gnulib's headers
 * and sources use on every other line.  All of it is compiler feature
 * detection done in the preprocessor -- it asks __GNUC__, __clang__,
 * __STDC_VERSION__ and __has_attribute, none of which this tcc claims, so
 * every one of these resolves to its portable fallback here.
 *
 * It is included verbatim rather than hand-stubbed because hand-stubbing it
 * is where the silent failures would be: an _GL_ATTRIBUTE_DEALLOC or
 * _GL_ASYNC_SAFE guessed with the wrong arity turns a declaration into
 * syntax that happens to parse.  Copied out of the tarball's own
 * config.h.in (lines 2741-3496), with the interleaved `#undef KNOB' lines
 * removed -- those are answers, and they are given in config.h, which
 * includes this file before it gives them.
 *
 * Nothing in here reads a findutils or gnulib knob; it is self-contained.
 */
#ifndef _GL_NT_COMMON_H
#define _GL_NT_COMMON_H
