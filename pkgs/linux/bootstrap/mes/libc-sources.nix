# The Mes C library's source lists, for x86-linux.
#
# Two sets: the MesCC ones, which are what MesCC itself can compile, and
# libc_gnu_SOURCES, the fuller set for a real C compiler -- tcc compiles that
# one once it exists.
#
# Mes computes these in build-aux/configure-lib.sh, a shell script that reads
# its own config.sh and appends to a variable per library.  Nothing in nix can
# run that, so the lists are recorded here, as nixpkgs' minimal-bootstrap
# records the same lists in a sources.json.
#
# Regenerate by sourcing that script with the same variables the build uses:
#
#   mes_libc=mes mes_cpu=x86 mes_kernel=linux compiler=mescc \   # or gcc
